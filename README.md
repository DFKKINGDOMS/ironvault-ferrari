...
- Read-only Vintage Parts inventory questions in the same command box. An exact part-number question reads the inventory authority even when that key has no catalog record, while clearly withholding OEM identity and fitment. A year/model request is filtered through exact GM catalog applications and is labeled as a verified subset whenever catalog coverage is incomplete; the response exposes mapped and total inventory-key counts and cannot claim completeness from a partial join. Results remain sortable, searchable and downloadable with quantity, source unit price, extended source inventory value and available catalog evidence. Source value is explicitly quantity × Vintage source price—not resale or eBay market value—and no draft or free-launch allowance is consumed.

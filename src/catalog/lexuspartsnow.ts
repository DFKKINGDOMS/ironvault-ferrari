...
function validatePartNumber(value: string): string {
  ...
  if (!/^[A-Z0-9][A-Z0-9-]{3,38}[A-Z0-9]$/.test(trimmed)) {
    throw new Error('Enter an exact Lexus part number using only letters, numbers and hyphens.');
  }
  return trimmed;
}
...
function safeDealerUrl(value: string): string | undefined {
  try {
    const url = new URL(value, LEXUS_PARTS_NOW_ORIGIN);
    if (url.protocol !== 'https:' || url.hostname !== 'www.lexuspartsnow.com') return undefined;
    return url.toString();
  } catch {
    return undefined;
  }
...
function parseInitialStore(html: string): Record<string, unknown> {
  ...
  if (!match?.[1]) throw new Error('LexusPartsNow returned a page without readable catalog data.');
  const json = match[1]
  ...
  try {
    ...
    throw new Error('LexusPartsNow catalog data could not be parsed safely.');
  }
}

function parseFitment(value: unknown): LexusPartFitment[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((row) => {
    if (!Array.isArray(row)) return [];
    const yearMakeModel = stringValue(row[0]);
    if (!yearMakeModel) return [];
    const match = yearMakeModel.match(/^(\d{4})(?:-(\d{4}))?\s+Lexus\s+(.+)$/i);
    const fitment: LexusPartFitment = {
      make: 'Lexus',
      model: match?.[3]?.trim() || yearMakeModel.replace(/^\d{4}(?:-\d{4})?\s+Lexus\s+/i, '').trim(),
...
function parseImages(partNumber: Record<string, unknown>, partInfo: Record<string, unknown>): LexusPartImage[] {
  ...
  const addImages = (value: unknown, type: LexusPartImage['type']) => {
    ...
    for (const entry of value) {
      ...
      const url = safeDealerUrl(stringValue(image.largeImg) || stringValue(image.img) || '');
...
async function getJson(fetcher: FetchLike, url: URL): Promise<Record<string, unknown>> {
  const response = await fetcher(url, {
    headers: {
      accept: 'application/json',
      referer: `${LEXUS_PARTS_NOW_ORIGIN}/`,
      site: 'LPN',
      'user-agent': 'PartQuill/0.4 (+https://partquill.com)'
    },
    signal: AbortSignal.timeout(12_000)
  });
  if (!response.ok) throw new Error(`LexusPartsNow search failed with HTTP ${response.status}.`);
  return objectValue(await response.json());
}

async function getHtml(fetcher: FetchLike, url: string): Promise<string> {
  const response = await fetcher(url, {
    headers: {
  ...
  if (!response.ok) throw new Error(`LexusPartsNow product lookup failed with HTTP ${response.status}.`);
...
export async function researchLexusPart(
  ...
  const partNumberInput = validatePartNumber(requestedPartNumber);
  ...
  const searchUrl = new URL('/api/search/search-words', LEXUS_PARTS_NOW_ORIGIN);
  searchUrl.searchParams.set('searchText', partNumberInput);
  searchUrl.searchParams.set('isConflict', 'false');
  const searchPayload = await getJson(fetcher, searchUrl);
  const redirectPath = stringValue(objectValue(searchPayload.data).redirectUrl);
  if (!redirectPath) throw new Error(`No LexusPartsNow catalog result was found for ${partNumberInput}.`);
  const productUrl = safeDealerUrl(redirectPath);
  if (!productUrl || !new URL(productUrl).pathname.startsWith('/parts/')) {
    throw new Error('LexusPartsNow returned an unsafe or unexpected catalog URL.');
  }

  const store = parseInitialStore(await getHtml(fetcher, productUrl));
  ...
  const fitment = parseFitment(partNumber.fitVehicleList);
  ...
  return {
    ...
    images: parseImages(partNumber, partInfo),
    fitment,
